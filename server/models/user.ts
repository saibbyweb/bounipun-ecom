...
const expressAuth = async (req, res, next, usergroup, strictMode) => {
  ...
  if (session === false) {
    /* TODO: reset cookie */
    if (strictMode) res.send({ notAuthorized: true });
    else next();
    return;
  }
  ...
  if (req.body.lockCheck === true) {
    ...
    if (user.contentUnlock.status === true) {
      ...
      const validated = await unlockMethods.validateUnlockCode(
        user.contentUnlock.code,
        user._id
      );
      /* if valid, append unlock status to the user object */
      if (validated !== false) console.log("✅ Lock access granted");

      req.body.unlocked = validated === false ? false : true;
    } else console.log("🔸🔸 No unlock code applied!");
  }
...
export const methods = {
  ...
  sendInternationalOtp: async (countryDialCode, phoneNumber) => {
    ...
    if (verfication.status === "pending") otpSent = true;
  ...
  registerUser: async (profile) => {
    const { response: newUser, error } = await task(
      new model({ ...profile }).save() as any
    );
  ...
  registerSession: async (userId) => {
    ...
    const { response: newSession, error } = await task(
      new session({
        token,
        user: userId,
      }).save()
  ...
  async normalizePricing(cartItems) {
    ...
    for (const product of cartItems) {
      ...
      if (
        ...
        console.log(
          `✅ Product ${product.productName} not under any sale, returned as is.`
        );
      ...
      if (validSales[product.sale] === undefined) {
        /* fetch sale details */
        const saleDetails = await saleMethods.validateSale(product.sale);
      ...
      for (const code of currencyCodes) {
        const price: any = product.pricing[code];
        console.log(`🟡 Original ${code} Price: ${price}`);
        product.pricing[code] = parseInt(
          price - (price / 100) * discountPercentage
        );
        console.log(`🔵  Discounted ${code} Price: ${product.pricing[code]}`);
      }
  ...
  findCartItem(cart, cartItem) {
    ...
    let foundIndex = cart.findIndex((item) => {
      ...
      if (cartItem.variant !== null && cartItem.fabric !== null) {
        paramsToBeMatched =
          paramsToBeMatched &&
          item.variant.toString() === cartItem.variant.toString() &&
          item.fabric.toString() === cartItem.fabric.toString();
      }
  ...
  calculateShippingCharge(totalOrderQuantity, currency, globalConfig) {
    const {
      domesticShippingCharge: domestic,
      internationalShippingCharge: international,
    } = globalConfig;
  ...
  adjustPrice(currency, price, globalConfig) {
    if (currency === "INR") return price;
      ...
      const inflatedPrice =
        (price * globalConfig.currencyMultiplier) / globalConfig.dollarValue;
  ...
  async calculateOrderTotal(
    ...
    if (couponCode !== "") {
      const couponDetails = await this.calculateCouponDiscountValue(
        cartTotal,
        couponCode,
        currency
      );
      if (couponDetails === false) return false;
    ...
    const grandTotal =
      parseFloat(subTotal) + parseFloat(shippingCharge) + parseFloat(taxes);
  ...
  async createOrderPayload(
    ...
    if (currency === "INR") {
      ...
      if (razorpayOrder === false) return false;
      ...
      console.log(
        grandTotal,
        "-- ORDER_TOTAL_FROM_STRIPE",
        (grandTotal * 100).toFixed(2)
      );

      const stripePaymentIntent =
        await paymentMethods.createStripePaymentIntent({
          amount,
          currency,
  ...
  async getNextSequence() {
    /* generate new id  */
    const nextDoc: any = db.model("orderSequence").findOneAndUpdate(
      { seqId: "sw-seq" },
      {
        $inc: { sequence: 1 },
      },
  ...
  async placeOrder(gatewayToken, transactionId, gateway) {
    ...
    const paymentIntent: any = await paymentIntentMethods.fetchPaymentIntent(
      gatewayToken
    );
      ...
      items: cartItems.map((item) => {
        const itemPrice =
          paymentIntent.currency === "INR"
            ? item.price
            : item.pricing[paymentIntent.currency];
    ...
    const userOrdersUpdated: any = await db
      .model("users")
      .findOneAndUpdate(
        { _id: paymentIntent.createdBy },
    ...
    const listOfProducts = orderDetails.items.map((item) => ({
      _id: item.productId,
      quantity: item.quantity,
    }));
    /* update stock */
    ...
    if (coupon !== undefined) {
      await couponMethods.updateCouponLog(
        coupon.code,
        paymentIntent.currency,
        orderSaved.number
      );
    } else console.log("COUPON_NOT_APPLIED");
  ...
  async orderBelongsToUser(userId, orderId) {
    const orderBelongsToUser = await db
      .model("users")
      .findOne({ _id: userId, orders: orderId })
      .select("_id");
  ...
  async cancelOrder(orderId, subOrderId, reason, userId) {
    ...
    const cancelSubOrder: any = await db
      .model("orders")
      .findOneAndUpdate(subOrderFilter, {
        $set: {
