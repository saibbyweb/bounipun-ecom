<template>
<div @click="navigateToProductPage" class="product-card" :class="{
    ...
    <div class="main-image-container center">
        <!-- TODO: size was 165% -->
        <slideshow ref="slideshow" :images="slideshowImages" extraClass="search-slideshow" :dots="true" :mSlideWidth="48" :dSlideWidth="29" mSlideHeight="315px" dSlideHeight="400px" size="cover" />
    </div>

    <!-- product colors [images] -->
    <div class="flex v-center evenly col color-previews" :style="readyToShip ? `align-items: center;justify-content: flex-end;` : ''" v-if="!(collectionName === 'Escape' && $route.name === 'collections')">

        <div class="flex center">
            <div class="base-color-boxes">
                <div @click.stop="setActiveBaseColor(index)" class="color-image shadow" v-for="(preview, index) in baseColorImagesPreview.previews" :key="index" :style="`background-image: url(${getBaseColorPath(index)})`" :class="{ active: isActiveBaseColor(index) }"></div>
            </div>
    ...
    <div>
        <div v-if="product.type !== 'third-party'" class="variants-available center">
            <div v-for="(variant, index) in variantsAvailable" :key="index" class="variant">
                {{ variant }}
            </div>
        </div>
    </div>
