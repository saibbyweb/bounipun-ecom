<template>
  <div class="page center-col">
    ...
    <p class="desc otp-sent" v-if="otpSent">
      A one time password has been sent to your mobile number.
    </p>

    <!-- <button id="forgot-password" class="clear" style="align-self: flex-start;"> Forgot your password ? </button> -->
    <button class="action" @click="sendOtp()">
...
export default {
  ...
  methods: {
    async sendOtp() {
        const sendOtpRequest = await this.$post('/sendOtp', { dialCode: '+91', phoneNumber: '9906697711', purpose: 'login'});
        if(sendOtpRequest === false)
            return
